    ...
    <div class="row-categorie sm:w-[20rem] md:w-[30rem] w-auto">
      <div class="tres-puntos">
        <div class="flex flex-col">
          ...
          <label :id="promo.id" for="small-toggle" class="inline-flex relative items-center mb-5 cursor-pointer" role="button" :class="isLoading === true ? 'animate-pulse' : ''" @click="editPromoCode(promo)">
...
export default {
  ...
  methods: {
    ...
    async editPromo() {
      if (this.infoToEdit.discount >= 1 && this.infoToEdit.discount <= 100) {
        const payload = {
        ...
        await this.$axios.put(
          `/api/v1/admin/prom-code/${this.infoToEdit.id}`,
          payload
        )
    ...
    async deletePromo(id) {
      try {
        await this.$axios
          .delete(`/api/v1/admin/prom-code/${id}`)
          .then((response) => {
            this.$notify({
              title: 'Eliminado!',
              type: 'warn',
